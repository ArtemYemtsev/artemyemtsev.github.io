...
- Глобальные классы: `.container`, `.section-label`, `.button.--primary/--secondary/--pill`, `.rollup-link`. **`.underline-link` глобальным НЕ является** — он объявлен только в scoped-стилях [Header.vue](app/components/Layout/Header.vue), то есть в других компонентах это пустой класс. Для ссылок с эффектом используй глобальный `.rollup-link` (см. ниже).
