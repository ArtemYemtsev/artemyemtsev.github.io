...
- Иконки — SVG-спрайт: `<svg><use :xlink:href="`/sprite.svg#<icon-name>`" /></svg>`. Файл — [public/sprite.svg](public/sprite.svg). **Каждому `svg` со спрайтом обязателен `fill: currentColor`** — у путей внутри `<symbol>` своего `fill` нет, без правила они рисуются чёрными и на тёмном фоне пропадают (так и было в футере и в секции контактов).
...
- Глобальные классы: `.container`, `.section-label`, `.button.--primary/--secondary/--pill`, `.rollup-link`, `.social-link`. Класса `.underline-link` больше нет — он жил в scoped-стилях шапки и демо-секции, обе переписаны; для ссылок с эффектом используй `.rollup-link` (см. ниже).

### Слои (z-index)
Шкала — CSS-переменные в [_variables.scss](app/assets/styles/abstracts/_variables.scss): `--z-below: -1`, `--z-base: 1`, `--z-sticky: 50`, `--z-header: 100`, `--z-dropdown: 200`, `--z-overlay: 900`, `--z-modal: 1000`, `--z-toast: 1100`. **Числом `z-index` в компонентах не пиши** — только переменная, иначе слои разъезжаются (шапка уже пряталась под секциями с `z-index: 1`). Шапка — `--z-header`: поверх всего контента, но ниже модалок и тостов.

### Rollup — анимация кнопок и ссылок
`@mixin rollup()` в [_mixins.scss](app/assets/styles/abstracts/_mixins.scss) — ховер-«перемотка» подписи. Подключается на сам триггер (`.button`, `.rollup-link`), внутри ждёт два слоя-дубля: `.rollup__layer--base` и `.rollup__layer--hover` (второй обязан быть `aria-hidden="true"` — иначе скринридер читает подпись дважды). Слои лежат в одной grid-ячейке, ховер сдвигает оба на высоту строки.
- **Разметку руками не пиши** — её генерит компонент [Ui/Button.vue](app/components/Ui/Button.vue) → `<UiButton>`. Тег выбирается по props: `to` → `NuxtLink`, `href` → `<a>`, иначе `<button>`. Props: `variant` (`primary`/`secondary`), `type`, `disabled`, `label`, `icon` (имя иконки спрайта); подпись можно отдать и слотом. Слот рендерится в оба слоя — вложенный декор (стрелка и т.п.) продублируется, это нормально.
- **Паддинг вешай на `.rollup__layer`, не на триггер**: слой должен закрывать кнопку целиком, иначе на ховере из-под него выглядывает фон.
- Параметр `$clip: true` — «мастеркард»-вариант с раскрытием из центра; отдан глобальным модификатором `.button.--pill`.
- Ховер выключен на тач-устройствах (`@media (hover: hover)`), на `:disabled` и при `prefers-reduced-motion` (там отклик даётся цветом).
- **Scoped-стили родителя до слоёв внутри `<UiButton>` не дотягиваются** (у дочернего компонента свой `data-v`). Новые варианты кнопки заводи глобальным модификатором в [_buttons.scss](app/assets/styles/layout/_buttons.scss), а не в секции. Контент слота — исключение: он компилируется в scope родителя.
- Витрина вариантов — [DemoUI/Buttons.vue](app/components/Section/DemoUI/Buttons.vue) (блок `demo-ui-buttons`, в контенте не подключён).
- Соц-ссылки-иконки — тот же rollup, компонент [Ui/SocialLink.vue](app/components/Ui/SocialLink.vue) → `<UiSocialLink :href :icon :label>`. Он сам ставит `aria-label` (иконка без подписи = ссылка без доступного имени) и `rel="noopener noreferrer"` для внешних. Вид — глобальный `.social-link` в [_socials.scss](app/assets/styles/layout/_socials.scss).

### Обвязка: шапка и футер
- **Высота шапки — переменные** `--header-height` / `--header-gap` / `--header-offset` в [_variables.scss](app/assets/styles/abstracts/_variables.scss). `--header-offset` компенсирует `position: fixed` в `.page-content` и задаёт `scroll-padding-top`. Не подменяй компенсацию паддингом секции: раньше её давал `hero`, и markdown-посты блога (у них hero нет) уезжали под шапку.
- **Шапка** ([Header.vue](app/components/Layout/Header.vue), [_header.scss](app/assets/styles/layout/_header.scss)) — плавающая капсула: прозрачна на первом экране, при скролле (`> 24px`, класс `--scrolled`) включает стекло. Активный пункт — по `.router-link-exact-active`, а НЕ `.router-link-active`: последний матчится по префиксу и подсвечивал бы главную на всех страницах.
- **Мобильное меню** — полноэкранная панель [_menu.scss](app/assets/styles/layout/_menu.scss) внутри `<header>`: `--z-below` уводит её под бар (бургер остаётся кликабельным), но она всё ещё поверх контента, т.к. `.header` — свой stacking context на `--z-header`. Скролл страницы блокирует класс `body.is-menu-open` (вешается через `useHead({ bodyAttrs })`). Отступы держит `.container` — хардкодить `--container-offset-mobile` нельзя, на планшете он разъезжается с контентом.
- **Данные обвязки — из контента, не хардкод.** [useLocaleNav()](app/composables/useLocaleNav.ts) — навигация текущей локали (общая для шапки и футера). [useSiteContacts()](app/composables/useSiteContacts.ts) — контакты и соцсети из блока `contacts` на `/<locale>/contacts`, плюс `email` отдельно для CTA в футере. Новый контакт добавляется правкой JSON во всех трёх локалях — код не трогается.
...
- [x] ~~Footer хардкодит контакты/соцссылки~~ — контакты и навигация тянутся из контента через `useSiteContacts()` / `useLocaleNav()`; hero тоже перешёл на общий источник (там висели три соцссылки с `href="#"`). `github-social-icon` добавлен в `contacts` всех трёх локалей.
